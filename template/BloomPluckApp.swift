...
struct BloomPluckApp: App {
    ...
    init() {
        ...
        do {
            let container = try BloomPersistence.makeContainer()
            self.container = container
            _services = StateObject(wrappedValue: BloomServices(context: container.mainContext))
            BuiltInBloomSeeder.seedIfNeeded(context: container.mainContext)
        } catch {
